...
void eeprom_init()
{
    ...
    if (eeprom_read_byte((uint8_t*)EEPROM_ED_SHEET_ALERT) == 0xff) eeprom_update_byte((uint8_t *)EEPROM_MMU_LOAD_FAIL, 1);
    if (eeprom_read_word((uint16_t*)EEPROM_MMU_NR_EXTRUDERS) == 0xffff) eeprom_update_word((uint16_t *)EEPROM_MMU_LOAD_FAIL, 5);
